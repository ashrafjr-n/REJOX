...
| `localStorage`/`sessionStorage` `.getItem`/`.setItem`/`.removeItem`/`.clear` | `AsyncStorage` (`@react-native-async-storage/async-storage`) **or** `react-native-mmkv` — the Ask stage's `storage` answer decides | Medium | Two different problems behind one question. MMKV is **synchronous**, so it is a pure rename. AsyncStorage is **async**, so the rename is the easy half: every call site must also become awaited, and a `Promise` left where a `string` was is silent data corruption, not a type error. |
