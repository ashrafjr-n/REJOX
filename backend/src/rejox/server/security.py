...
``acct:<digest>``
    ...
    see :mod:`rejox.server.sessions`. Browsers get this because two of the surfaces they
...
(``REJOX_RATE_STORE``) the way ``rejox.server.queue`` decides where a migration runs:
...
# Redis connect/read ceiling, mirroring rejox.server.queue: this is a local-network hop,
...
class RateLimiter:
    ...
    def _store(self):
        ...
        from rejox.server.queue import redis_url  # noqa: PLC0415 - imported late by design
...
def guard_storage(identity: str) -> None:
    ...
    """
    from rejox.pipeline import workspace  # noqa: PLC0415 - avoids an import cycle
