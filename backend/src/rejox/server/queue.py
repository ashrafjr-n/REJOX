...
WHERE the work runs; ``rejox.server.jobs`` owns what the work is.
...
def env_int(name: str, default: int) -> int:
    ...
    Public because ``rejox.server.jobs`` reads its own ceilings the same way, and two
...
def _enqueue_thread(job_id: str, payload: dict[str, Any]) -> None:
    from rejox.server import jobs  # noqa: PLC0415 - circular at module scope
...
def running_count() -> Optional[int]:
    ...
    """
    if backend() != "rq":
        from rejox.server import jobs  # noqa: PLC0415
